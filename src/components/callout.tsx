...
import { CircleCheck, CircleX, Info, TriangleAlert, Rocket, Pickaxe, KeyRound } from 'lucide-react';
...
> & {
   ...
   * @defaultValue info
   */
  type?: 'info' | 'warn' | 'error' | 'success' | 'warning' | 'example' | 'todo' | 'proof';
...
export const Callout = forwardRef<HTMLDivElement, CalloutProps>(
  ({ className, children, title, type = 'info', icon, ...props }, ref) => {
    ...
    if(!title) title = type.at(0)?.toUpperCase() + type.slice(1);
        ...
        {icon ??
          {
            ...
            example: <Rocket className={iconClass} />,
            todo: <Pickaxe className={iconClass} />,
            proof: <KeyRound className={iconClass} />,
          }[type]}
