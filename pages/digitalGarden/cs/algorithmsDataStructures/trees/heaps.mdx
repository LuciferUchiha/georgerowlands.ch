...
problem for this is when implementing the [selection sort algorithm](/digitalGarden/cs/algorithmsDataStructures/sorting/selectionSort). 
...
Using a heap to implement the selection sort algorithm then leads to the [heap sort algorithm](/digitalGarden/cs/algorithmsDataStructures/sorting/heapSort) which runs in $O(n \log n)$ time.
