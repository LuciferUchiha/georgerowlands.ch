...
> $|h_r(v) - h_l(v)| \leq 1$ or $|bal(v)| \leq 1$ for all nodes $v$ in the tree.
...
balance factors of the ancestors and recursively check if the AVL property is still satisfied and if not perform the necessary corrections.
