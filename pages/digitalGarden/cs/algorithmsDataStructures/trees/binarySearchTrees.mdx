...
parent, we enforce a similar structure in the binary search tree by making sure that the tree maintains the following properties:
...
Otherwise we repeat this process until we reach a leaf node. We then add the new node as a child of the leaf node.
...
the tree complete like in a heap to keep the height as low as possible.
