...
two vertical bars, e.g. $\|\boldsymbol{x}\|$, and the subscript denotes the type of norm, e.g. $\|\boldsymbol{x}\|_1$ or $\|\boldsymbol{x}\|_2$ etc.

### Absolute Value

The absolute value of a number is the distance of that number from zero on the number line. It is always positive, or zero if the number is zero.
...
If $x = 5$, then $|x| = 5$. If $x = -5$, then $|x| = -(-5) = 5$.
</Callout>

### Manhattan Norm

The Manhattan norm or $L_1$ norm is defined as the sum of the absolute values of the vector's components. It is called the Manhattan norm because it is the distance 
...
<Image
    src="/maths/manhattenNorm.png"
    caption="As we can see no matter how we move along the roads of Manhattan, the distance between two points is always the same, the red and green path are the same length."
    width={400}
    />
...
<Image
    src="/maths/linearDependence2d.png"
...
<Image
    src="/maths/scalarMultiplication.png"
    caption="Scalar multiplication in 2D."
    width={400}
    />
...
<Image
    src="/maths/normalizedVectors.png"
...
<Image
    src="/maths/orthonormalVectors.png"
...
Something with orthonormal basis and quantum computing
