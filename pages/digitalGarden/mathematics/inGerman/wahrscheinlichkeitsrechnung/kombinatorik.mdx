...
Die **Fakultät** ist eine Funktion welche nur für nicht negativen ganzen Zahl $n$ definiert ist. Die fakultät einer Zahl
wird geschrieben als $n!$, und ist das Produkt aller positiven ganzen Zahl kleiner gleich $n$.
...
Eine wichtige und einzige Ausnahme für diese funktion ist, dass $0! = 1$ ist.
</Callout>

## Binomialkoeffizient

Der Binomialkoeffizient ist eine mathematische Funktion welches oftmals in verschiedenen Themengebieten der Mathematik
auftaucht.

Die Formel für ein Binomialkoeffizient oder auch kurz Binom ist definiert als
...
In der Wahrscheinlichkeitsrechnung gibt ein Binom an, auf wie viele verschiedene Arten man $k$ Objekte
aus einer Menge von $n$ verschiedenen Objekten auswählen kann (ohne Zurücklegen und ohne die Reihenfolge zu beachten).
Formel gesagt ist also der Binomialkoeffizient die Anzahl der $k$-element Teilmengen einer $n$-element Menge.

Auf Taschenrechnern findet man oftmals die englische Abkürzung **nCr**, welche für **n choose r** steht und dem Binom entspricht.

### Für Binomische Terme

Ein binomischer term hat die folgende Form

$$
(x + y)^n , n \in \mathbb{N}
$$

die Binomialkoeffizienten geben für einen binomischen term die Koeffizienten der Ausmultiplikation an. Dazu verwendet
...
Das **Pascalsche Dreieck** ist eine grafische Darstellung der Binomialkoeffizienten, welches auch die Berechnung vereinfacht.
Dabei zeichnet man ein Dreieck wo die obere variable $n$ des Binomialkoeffizienten dem Zeilenindex und die untere variable
$k$ dem Spaltenindex entspricht.

![PascalsTriangle](/maths/pascalsTriangle.png)

Die Berechung der Werte ist dann ganz einfach. Zuerst setzt man für alle Elemente am Rand des Dreiecks eine 1 ein, weil
ein Binom mit $0$ oder $n$ als untere variable sich zu 1 auflösen lässt. Danach ist jeder restliche Wert die Summe der
Werte genau über sich. Diese Herleitung kommt von der Gleichung
...
<Callout type="example" title="Beispiel für Pascalsches Dreieck Herleitung">
