...
Instead of looking for an index of a certain element we can also use the `findIndex()` or `findLastIndex()` methods which work very similiar but instead of looking for an element are given a predicate function and return the index of the element that matches that criteria first or last.
