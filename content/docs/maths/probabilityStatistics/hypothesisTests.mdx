...
```math
\Var[Z_i] = \Var[X_i - Y_i] = \Var[X_i] + \Var[Y_i] = \sigma^2 + \sigma^2 = 2\sigma^2
```
...
```math
\Var(D) = V(\bar{X}_n - \bar{Y}_m) = V(\bar{X}_n) + V(\bar{Y}_m) = \frac{\sigma^2}{n} + \frac{\sigma^2}{m} = \sigma^2 \left(\frac{1}{n} + \frac{1}{m}\right)
```
