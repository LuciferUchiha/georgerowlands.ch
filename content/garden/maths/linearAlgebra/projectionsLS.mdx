...
The projection of a vector $\mathbf{b} \in \mathbb{R}^m$ onto a subspace $S$ of $\mathbb{R}^m$ is the point $\mathbf{p} \in S$ that is 
closest to $\mathbf{b}$. So we would define the projection as follows:
...
by a single basis vector $\mathbf{a}$. So $S = \{\lambda \mathbf{a} : \lambda \in \mathbb{R} \, \mathbf{a} \in \mathbb{R}^2\}$ = C($\mathbf{a}$).
If we visualize this, we can see that the projection of $\mathbf{b}$ onto the line, i.e the point $\mathbf{p}$ that is closest to 
$\mathbf{b}$ is there where the error vector $\mathbf{e} = \mathbf{b} - \mathbf{p}$ is orthogonal to the line. 
...
So how do we find the point $\mathbf{p}$ using this information. We can also observe that $\mathbf{p}$ is a scalar multiple of $\mathbf{a}$, 
i.e $\mathbf{p} = \lambda \mathbf{a}$. So we can write the error vector $\mathbf{e}$ as $\mathbf{e} = \mathbf{b} - \lambda \mathbf{a}$.
...
So we get some function $g(\lambda)$ that we want to minimize, the values of $||\mathbf{b}||^2$ and $||\mathbf{a}||^2$ and $\mathbf{b}^T\mathbf{a}$ are all constants. 
...
So we have found the formula for $\lambda$ which is the scalar multiple of $\mathbf{a}$ that gives us the projection of $\mathbf{b}$ onto the line. 
...
We prefer to have the $\mathbf{b}$ term seperated and we can move around the multiplication of. So the nominator is then the outer product of $\mathbf{a}$ with itself and 
the denominator is the inner product of $\mathbf{a}$ with itself. This then gives us the final formula for the projection of $\mathbf{b}$ onto the line spanned by $\mathbf{a}$.
...
&\implies \mathbf{a}^T(\mathbf{b} - \text{proj}_S(\mathbf{b})) = 0 \\
...
In the general case, we want to project a vector $\mathbf{b} \in \mathbb{R}^m$ onto a subspace $S \subseteq \mathbb{R}^m$ of dimension $n$, 
where $n \leq m$. This subspace $S$ is spanned by $n$ linearly independent vectors, which we organize as the columns of a matrix $\mathbf{A} 
\in \mathbb{R}^{m \times n}$. The column space of $\mathbf{A}$ is then the subspace $S=C(\mathbf{A})$. Our goal is 
to then find $\text{proj}_S(\mathbf{b}) = \mathbf{p}$, the projection of $\mathbf{b}$ onto $S$, such that $\mathbf{p}$ is the closest 
point in $S$ to $\mathbf{b}$.

Any vector $\mathbf{p}$ in the subspace $S$ can be expressed as a linear combination of the columns of $\mathbf{A}$. If we store the coefficients 
of this linear combination in a vector $\hat{\mathbf{x}} \in \mathbb{R}^n$, we can write the projection as:
```math
\mathbf{p} = \mathbf{A}\hat{\mathbf{x}}.
```

Just like in the 1D case, we want to minimize the error vector. This is then equivalent to finding $\hat{\mathbf{x}}$ such that the error vector 
$\mathbf{e} = \mathbf{b} - \mathbf{p}$ is minimized. Specifically, we minimize the squared norm of the error:
```math
...
```math
||\mathbf{e}||^2 = (\mathbf{b} - \mathbf{A}\hat{\mathbf{x}})^T (\mathbf{b} - \mathbf{A}\hat{\mathbf{x}})
= \mathbf{b}^T\mathbf{b} - 2\hat{\mathbf{x}}^T\mathbf{A}^T\mathbf{b} + \hat{\mathbf{x}}^T\mathbf{A}^T\mathbf{A}\hat{\mathbf{x}}.
```

This gives us a function $g(\hat{\mathbf{x}})$ very similar to the 1D case that we want to minimize.
To minimize this function with respect to $\hat{\mathbf{x}}$, we compute the gradient with respect to $\hat{\mathbf{x}}$ and set it to zero:
```math
...
Because we know that the columns of $\mathbf{A}$ are linearly independent as they form a basis for the subspace $S$, the matrix $\mathbf{A}^T\mathbf{A}$ is square and invertible. 
...
the rank of $\mathbf{A}$ is $n$, the rank of $\mathbf{A}^T\mathbf{A}$ is also $n$ and therefore it is now full rank and invertible. In the general case, 
the matrix $\mathbf{A}$ is not square and therefore not invertible. So if left multiply the normal equation with the inverse we can solve for $\hat{\mathbf{x}}$:

```math
\hat{\mathbf{x}} = (\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T\mathbf{b}
```

Substituting $\hat{\mathbf{x}}$ back into $\mathbf{p} = \mathbf{A}\hat{\mathbf{x}}$, we obtain the projection of $\mathbf{b}$ 
onto $S$:
...
the matrix $\mathbf{P} = \mathbf{A}(\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T$ we get the so called **projection matrix** for the 
...
We know that matrices define linear transformations. Therefore the projection matrix $\mathbf{P}$ defines a linear transformation that projects any vector 
...
Where the vector $\mathbf{b}$ is of dimension $m$ and the subspace and therefore the projection is of dimension $n$. The projection transformation 
...
So with the projection matrix we should have $\mathbf{P}^2b = \mathbf{P}b$ which is the same as $\mathbf{P}^2 = \mathbf{P}$.
...
```math
\text{proj}_{S^{\perp}}(\mathbf{b}) = (\mathbf{I} - \mathbf{P})\mathbf{b}
```

## Orthogonal Matrices

An **orthogonal matrix** is a matrix whose columns $\mathbf{q}_1, \mathbf{q}_2, \ldots, \mathbf{q}_n$
...
2. They are all orthogonal to each other, meaning $\mathbf{q}_i^T \mathbf{q}_j = 0$ for $i \neq j$ the dot products where 
...
An orthogonal matrix $\mathbf{Q} \in \mathbb{R}^{m \times n}$ has orthonormal columns then it has some very nice properties. 
...
If we have the orthogonal matrix $\mathbf{Q} \in \mathbb{R}^{m \times n}$ with the orthonormal columns $\mathbf{q}_1, \mathbf{q}_2, \ldots, \mathbf{q}_n$. 
When we compute $\mathbf{Q}^T \mathbf{Q}$, the $(i, j)$-entry of the resulting matrix is:
...
Which then means that $\mathbf{Q}$ is invertible, and its inverse is given by:
...
$\mathbf{Q} \in \mathbb{R}^{n \times n}$ and let $\mathbf{x} \in \mathbb{R}^n$ be any vector. So if we consider
 the transformed vector $\mathbf{y} = \mathbf{Q} \mathbf{x}$. Then the norm of $\mathbf{y}$ is:
...
preserves the inner product of two vectors. The inner product of two vectors $\mathbf{x}, \mathbf{z} \in \mathbb{R}^n$ is defined as:

```math
\langle \mathbf{x}, \mathbf{z} \rangle = \mathbf{x}^T \mathbf{z}.
```

Now consider the transformed vectors $\mathbf{y} = \mathbf{Q} \mathbf{x}$ and $\mathbf{w} = \mathbf{Q} \mathbf{z}$. The inner product of $\mathbf{y}$ and $\mathbf{w}$ is:

```math
\langle \mathbf{y}, \mathbf{w} \rangle = \mathbf{y}^T \mathbf{w} = (\mathbf{Q} \mathbf{x})^T (\mathbf{Q} \mathbf{z}).
```
...
Suppose we are given $\mathbf{Q} \in \mathbb{R}^{m \times n}$ that has orthonormal columns. We now want to project a vector 
$\mathbf{b} \in \mathbb{R}^m$ onto subspace spanned by the columns of $\mathbf{Q}$, so $C(\mathbf{Q})$. We have already seen that 
the projection of $\mathbf{b}$ onto $C(\mathbf{Q})$ is given by:

```math
\text{proj}_{C(\mathbf{Q})}(\mathbf{b}) = \mathbf{Q} \hat{\mathbf{x}},
```

Which becomes the normal equations. However, since $\mathbf{Q}$ has orthonormal columns, we can simplify the normal 
equations and the projection matrix. 

```math
\begin{align*}
\mathbf{Q}^T \mathbf{Q} \hat{\mathbf{x}} = \mathbf{Q}^T \mathbf{b} \\
\hat{\mathbf{x}} = \mathbf{Q}^T \mathbf{b} \\
\\
\text{proj}_{C(\mathbf{Q})}(\mathbf{b}) = \mathbf{Q} \hat{\mathbf{x}} = \mathbf{Q} (\mathbf{Q}^T \mathbf{b})
\end{align*}
```

The projection matrix for $C(\mathbf{Q})$ is then simply:
...
- The matrix containing the features is $\mathbf{A} = \begin{bmatrix} 1 & t_1 \\ 1 & t_2 \\ \vdots & \vdots \\ 1 & t_n \end{bmatrix}$.

The reason for the column of ones is that when we multiply the matrix $\mathbf{A}$ with the vector $\mathbf{x}$ we want to just add the 
...
additional feature, the matrix $\mathbf{A}$ would include an additional column corresponding to this feature.

More generally, for a given matrix $\mathbf{A} \in \mathbb{R}^{m \times n}$ (where $m$ is the number of observations and $n$ is the 
number of features plus one for the intercept) and a vector $\mathbf{b} \in \mathbb{R}^m$, we aim to find the vector 
$\mathbf{x} \in \mathbb{R}^n$ that minimizes:
...
the projection of $\mathbf{b}$ onto the column space of $\mathbf{A}$:

```math
\min_{\mathbf{x} \in \mathbb{R}^n} \|\mathbf{b} - \mathbf{A}\mathbf{x}\|^2 = \|\mathbf{b} - \text{proj}_{C(\mathbf{A})}(\mathbf{b})\|^2.
```

This actually makes sense as we are given some vector and we are trying to create it is a linear combination of the columns of $\mathbf{A}$. 

Because we have now noticed that we are looking for the projection of $\mathbf{b}$ onto the column space of $\mathbf{A}$ we can use 
...
To solve the least squares problem, the columns of $\mathbf{A}$ must be **linearly independent**. Linear independence ensures that 
$\mathbf{A}^T\mathbf{A}$ is invertible, which is crucial for deriving the solution. For example, if all the feature values $t_k$ are 
identical (e.g., $t_i = t_j$ for all $i \neq j$), then the column would be a multiple of the first column and the columns of $\mathbf{A}$ 
would not span a sufficiently large space, making it impossible to uniquely determine the coefficients $\mathbf{x}$. Linear independence of 
the columns of $\mathbf{A}$ ensures that the data provides enough information to determine a unique solution.

<Callout type="example">
For the case of predicting house prices based on size, the solution is given by:

```math
\begin{bmatrix} a_0 \\ a_1 \end{bmatrix} = (\mathbf{A}^T\mathbf{A})^{-1}\mathbf{A}^T\mathbf{b}.
```
