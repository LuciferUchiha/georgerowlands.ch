...
{{< callout type="info" title="Connection to Linear Autoencoders" >}}
PCA can be reached from a complementary direction. A [linear autoencoder](/garden/ml/computerVision/autoencoders#linear-autoencoders) compresses data through a bottleneck and is trained to minimize the squared reconstruction error. For centered data, minimizing that reconstruction error turns out to be exactly equivalent to maximizing the variance of the projection, so the optimal linear autoencoder projects onto the same principal subspace derived here. The two viewpoints, minimizing reconstruction error and maximizing retained variance, are one and the same.
{{< /callout >}}

## Variance and the Covariance Matrix

When we project high-dimensional vectors onto a direction $u$, we get scalars $u^T x_i$. These are just numbers (the coordinates along that direction), and we can compute their variance in the usual way. So while our data points $x_i$ are vectors, the projected values are scalars, and it is the variance of these scalar projections that we want to maximize. The covariance matrix $\Sigma$ provides a compact way to compute this variance for any direction $u$ using the quadratic form $u^T \Sigma u$.
...
This matrix captures how each pair of dimensions varies together. The diagonal entry $\Sigma_{jj}$ is the variance of the $j$-th dimension, while the off-diagonal entry $\Sigma_{jk}$ is the covariance between dimensions $j$ and $k$.

The covariance matrix is [**symmetric**](/garden/maths/linearAlgebra/eigendecomposition#eigenvalues-and-eigenvectors-of-symmetric-matrices) (since $\Sigma_{jk} = \Sigma_{kj}$) and [**positive semi-definite**](/garden/maths/linearAlgebra/eigendecomposition#positive-definite-and-positive-semi-definite-matrices) (since $v^T \Sigma v = \frac{1}{n} \sum_{i=1}^n (v^T(x_i - \bar{x}))^2 \geq 0$ for any vector $v$).
...
$$
\text{Var}(\pi(X)) = \frac{1}{n} \sum_{i=1}^n (u_1^T x_i)^2 = \frac{1}{n} \sum_{i=1}^n u_1^T x_i x_i^T u_1 = u_1^T \left( \frac{1}{n} \sum_{i=1}^n x_i x_i^T \right) u_1 = u_1^T \Sigma u_1
$$

Therefore, maximizing variance is equivalent to maximizing the quadratic form $u_1^T \Sigma u_1$ subject to the constraint $u_1^T u_1 = 1$.
...
This tells us that the optimal direction $u_1$ is an [**eigenvector**](/garden/maths/linearAlgebra/eigendecomposition) of the covariance matrix $\Sigma$, and the corresponding [eigenvalue](/garden/maths/linearAlgebra/eigendecomposition) $\lambda$ equals the variance along that direction (since $u_1^T \Sigma u_1 = u_1^T \lambda u_1 = \lambda$).

To maximize variance, we should choose $u_1$ to be the eigenvector corresponding to the **largest eigenvalue** $\lambda_1^*$ of $\Sigma$. This eigenvector is called the **first principal component**.

{{< callout type="proof" title="Largest Eigenvalue Maximizes Variance" >}}
We need to verify that among all eigenvectors of $\Sigma$, the one with the largest eigenvalue gives the maximum variance.

Suppose $\Sigma$ has eigenvalues $\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_D$ with corresponding unit eigenvectors $u_1, u_2, \ldots, u_D$. The [spectral theorem](/garden/maths/linearAlgebra/eigendecomposition) tells us these eigenvectors form an orthonormal basis for $\mathbb{R}^D$.
...
$$
v^T \Sigma v = v^T \left( \sum_{i=1}^D \lambda_i \alpha_i u_i \right)
$$

The key insight is that $\Sigma v = \Sigma(\sum_{i=1}^D \alpha_i u_i) = \sum_{i=1}^D \alpha_i \Sigma u_i = \sum_{i=1}^D \alpha_i \lambda_i u_i$ (using $\Sigma u_i = \lambda_i u_i$).

Taking the inner product with $v$:
$$
v^T \Sigma v = \sum_{i=1}^D \alpha_i \lambda_i (u_i^T v) = \sum_{i=1}^D \lambda_i \alpha_i^2
$$

Since $\lambda_1$ is the largest eigenvalue, we have $\lambda_i \leq \lambda_1$ for all $i$. Therefore:
$$
v^T \Sigma v = \sum_{i=1}^D \lambda_i \alpha_i^2 \leq \lambda_1 \sum_{i=1}^D \alpha_i^2 = \lambda_1
$$
...
1. Find the first principal component $u_1^*$ as the eigenvector of $\Sigma$ with the largest eigenvalue $\lambda_1^*$.

2. To find the second principal component, we want a direction orthogonal to $u_1^*$ that has maximum variance. Define $X_1 = \{x - (u_1^{*T} x) u_1^* : x \in X\}$ as the dataset with projections onto $u_1^*$ removed. Compute the covariance matrix $\Sigma_1$ of $X_1$ and find its largest eigenvector $u_2^*$.
...
However, there is a more elegant formulation. The principal components $u_1^*, \ldots, u_d^*$ are simply the eigenvectors corresponding to the $d$ largest eigenvalues of $\Sigma$. These eigenvectors are [automatically orthogonal](/garden/maths/linearAlgebra/eigendecomposition#eigenvalues-and-eigenvectors-of-symmetric-matrices) (since $\Sigma$ is symmetric).

Therefore we just need to find the eigenvectors $u_1^*, \ldots, u_d^*$ corresponding to the largest eigenvalues $\lambda_1^* \geq \lambda_2^* \geq \cdots \geq \lambda_d^*$ of $\Sigma$. These form the **first d principal components** of the data. The projection $\pi: \mathbb{R}^D \to \mathbb{R}^d$ is then given by:
...
The [spectral theorem](/garden/maths/linearAlgebra/eigendecomposition) states that any real symmetric matrix $\Sigma$ can be decomposed as:

$$
\Sigma = U \Lambda U^T
$$

where $U = [u_1, \ldots, u_D]$ is an orthogonal matrix whose columns are the eigenvectors of $\Sigma$, and $\Lambda = \text{diag}(\lambda_1, \ldots, \lambda_D)$ is a diagonal matrix of eigenvalues ordered such that $\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_D$. The eigenvectors $\{u_i\}$ form an orthonormal basis, meaning $u_i^T u_j = \delta_{ij}$ (where $\delta_{ij}$ is the Kronecker delta: 1 if $i = j$ and 0 otherwise).
...
$$
\text{Var}_{\text{total}} = \text{tr}(\Sigma) = \sum_{j=1}^D \Sigma_{jj} = \sum_{j=1}^D \lambda_j
$$

where $\text{tr}(\Sigma)$ is the trace of $\Sigma$ (the sum of diagonal elements). The trace of a matrix equals the sum of its eigenvalues, which follows from the [spectral theorem](/garden/maths/linearAlgebra/eigendecomposition). Intuitively, the trace $\sum_{j=1}^D \Sigma_{jj}$ sums the variance along each original coordinate axis. The spectral theorem tells us this same total variance can be decomposed into variance along the eigenvector directions, which sum to $\sum_{j=1}^D \lambda_j$.
