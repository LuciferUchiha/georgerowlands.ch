...
Show why it is useful for solving linear equations Ax=b

Derive formulas for 2x2 case. Show that if rank 0 or 1 then no inverse satisifes the formula because division by zero.

generlalize as to why it must be full rank, this also means that it must be square.

can put inverse on either side, why? So we can write $A^{-1}Ax = A^{-1}b$ or $xA^{-1}A = bA^{-1}$, so we can multiply by the inverse on either side?
...
Derive (AB)^-1 using linear transformations?
