...
```mathP(|X-\mu|\geq k) \leq \frac{\sigma^2}{k^2}, \text{ für k > 0}```

daraus folgt dann auch

```mathP(|X-\mu| < k) \leq 1 - \frac{\sigma^2}{k^2}```
...
:::note Beispiel Ungleichung von Tschebyscheff
 ...
 ```mathP(|X -175cm| \geq 15) \leq \frac{100}{15^2} \approx 44.4 \%```

 Tatsächlich mit Normalverteilung:

 ```mathP(|X -175cm| \geq 15) = 1- (normcdf(190,175,10) - normcdf(160,175,10)) \approx 13.4 \%```
