...
Just as with [estimators]() and [confidence intervals](), we start with sample data $x_1, x_2, \ldots, x_n$, viewed as realizations of random variables $X_1, \ldots, X_n$ drawn from some population distribution in a family $\mathbb{P}_\theta$, parameterized by an unknown $\theta$.
...
$$
\mathbb{P}_\theta(T \in K) \leq \alpha
$$
...
$$
\mathcal{L}(x_1, x_2, \ldots, x_n; \theta) = \begin{cases}
...
$$
R(x_1, \ldots, x_n) = \frac{\mathcal{L}(x_1, \ldots, x_n \mid \theta_A)}{\mathcal{L}(x_1, \ldots, x_n \mid \theta_0)}
$$

By convention if $\mathcal{L}(x_1, \ldots, x_n \mid \theta_0) = 0$ then $R(x_1, \ldots, x_n) = \infty$. The likelihood ratio test uses this as its statistic:
...
$$
R(x_1, \ldots, x_n) = \frac{\sup_{\theta \in \Theta_A} \mathcal{L}(x_1, \ldots, x_n \mid \theta)}{\sup_{\theta \in \Theta_0} \mathcal{L}(x_1, \ldots, x_n \mid \theta)}
$$
...
$$
\mathcal{L}(x_1, \ldots, x_n \mid \theta) = \prod_{i=1}^n p_X(x_i; \theta) = \theta^{\sum_{i=1}^n x_i} (1 - \theta)^{n - \sum_{i=1}^n x_i}
$$

which can be rewritten using $S_n = \sum_{i=1}^n x_i$:

$$
\mathcal{L}(x_1, \ldots, x_n \mid \theta) = \theta^{S_n} (1 - \theta)^{n - S_n}
$$
...
$$
\mathcal{L}(x_1, \ldots, x_n \mid H_A) = \theta_A^{S_n} (1 - \theta_A)^{n - S_n}
$$
...
$$
\begin{align*}
R(x_1, \ldots, x_n; \frac{1}{2}, \theta_A) &= \frac{\mathcal{L}(x_1, \ldots, x_n \mid H_A)}{\mathcal{L}(x_1, \ldots, x_n \mid H_0)} \\
...
$$
\mathcal{L}(x_1, \ldots, x_n \mid \theta) = \prod_{i=1}^n f_X(x_i, \theta) = \left(\frac{1}{\sqrt{2 \pi \sigma^2}}\right)^n \exp\left(-\frac{1}{2 \sigma^2} \sum_{i=1}^n (x_i - \theta)^2\right)
$$
...
$$
\mathbb{P}_{\theta_0}(Z \in K) = \mathbb{P}_{\theta_0}(Z > c_{(>)} ) = \alpha
$$

Which for the standard normal distribution gives us:

$$
\mathbb{P}_{\theta_0}(Z > c_{(>)} ) = 1 - \Phi(c_{(>)} ) = \alpha
$$
...
$$
\mathbb{P}_{\theta_0}(Z < c_{(<)} ) = \Phi(c_{(<)} ) = \alpha
$$
...
$$
\mathbb{P}_{\theta_0}(Z \in K) = \mathbb{P}_{\theta_0}(|Z| > c_{(=)} ) = \mathbb{P}_{\theta_0}(Z < c_{(=)} ) + \mathbb{P}_{\theta_0}(Z > c_{(=)})= \alpha
$$

Because the standard normal distribution is symmetric, we have:

$$
\mathbb{P}_{\theta_0}(Z < c_{(=)} ) = \mathbb{P}_{\theta_0}(Z > c_{(=)}) = \frac{\alpha}{2}
$$
...
$$
\mathbb{E}[Z_i] = \mathbb{E}[X_i - Y_i] = \mathbb{E}[X_i] - \mathbb{E}[Y_i] = \mu_X - \mu_Y
$$
- The variance is given by the properties of variance for independent random variables:

$$
	\text{Var}[Z_i] = 	\text{Var}[X_i - Y_i] = 	\text{Var}[X_i] + 	\text{Var}[Y_i] = \sigma^2 + \sigma^2 = 2\sigma^2
$$
...
| Patient | Before ($X_i$) | After ($Y_i$) | $Z_i = X_i - Y_i$ |
| ------- | :---------------: | :--------------: | :--------------------: |
| 1       |        140        |        130       |           10           |
| 2       |        135        |        132       |            3           |
| 3       |        150        |        140       |           10           |
...
We compute the sample mean $Z_i$:
...
Then we compare $T$ to $t_{5,1-\alpha/2}$ for your desired significance level to make a decision. 
{{< /callout >}}
...
$$
\mathbb{E}(D) = \mathbb{E}(\bar{X}_n - \bar{Y}_m) = \mathbb{E}(\bar{X}_n) - \mathbb{E}(\bar{Y}_m) = \mu_X - \mu_Y
$$
...
$$
	\text{Var}(D) = V(\bar{X}_n - \bar{Y}_m) = V(\bar{X}_n) + V(\bar{Y}_m) = \frac{\sigma^2}{n} + \frac{\sigma^2}{m} = \sigma^2 \left(\frac{1}{n} + \frac{1}{m}\right)
$$
...
{{< callout type="example" title="Tossing a Coin" >}}
...
$$
T(\omega) = \frac{X_n(\omega)- n\theta_0}{\sqrt{n\theta_0(1-\theta_0)}} 
= \frac{60-50}{\sqrt{100\times0.5\times0.5}} 
= \frac{10}{5} = 2.
$$
...
$$
p-value(\omega) = P(Z \leq -2) + P(Z \geq 2)
= 2P(Z\geq 2) = 2[1-\Phi(2)],
$$
...
$$
\gamma \mathbb{P}_\theta(T > c) + (1 - \gamma) \mathbb{P}_\theta(T > c+1) = \alpha
$$
...
$$
\gamma = \frac{\alpha - \mathbb{P}_\theta(T > c+1)}{\mathbb{P}_\theta(T > c) - \mathbb{P}_\theta(T > c+1)} \approx 0.893
$$
{{< /callout >}}