...
<Callout type="todo">
What is the the difference between probability and stochastic. So saying something is probabilist or something is stochastic. I think Probabilistic just means that there is some form of randomness involved so it is random or not deterministic. Stochastic on the other hand refers to processes that are random, so multiple steps. So in the case of stochastic independence we are talking about multiple steps, so multiple events that are independent of each other. So in other words stochastic independence is a special case of probabilistic independence.

Hence we say we have stochastic processes rather than probabilistic processes. However, this is not always the case and sometimes we use the terms interchangeably.
</Callout>

### Physical Independence vs Stochastic Independence

It is important to note that stochastic independence does not have to mean that the events are physically independent of each other. Two events can be indepdent of each other despite the experiment only being conducted once physically. 
...
A set of events $(A_i)_{i \in I}$ is called **stochastically independent** if $I$ is finite and for all $J \subseteq I$ we have:

```math
\P\left(\bigcap_{i \in J} A_i\right) = \prod_{i \in J} \P(A_i)
```

So any subset of the events is independent of each other. This means that the events do not influence each other and can be done independently of each other. Simply checking all the pairs of events is not enough to establish independence. So if we have three events $A$, $B$, and $C$, we need to check all the dependencies between them to be able to say that they are independent of each other:
...
<Callout type="todo">
This was shown in algorithms and probability. We need to check all the dependencies to be able to say that the events are independent of each other. This becomes computationaly very expensive as the number of events increases. Can this also be done more efficiently with dp?
</Callout>

<Callout type="example">
Lets consider the example of us throwing two dice and the following events:

- $A$: The first die is even, $\{\omega | \omega_1 \in \{2,4,6\}\}$
- $B$: The second die is odd, $\{\omega | \omega_2 \in \{1,3,5\}\}$
- $C$: Both dice are smaller then or equal to 2, $\{\omega | \omega_1 \leq 2 \land \omega_2 \leq 2\}$

Check all the dependencies (suprisngly A and C are independent)
</Callout>
...
<Callout type="todo">
Explain this in more detail and link it to the multiplication rule and countable additivity. As to why we multiply things or add things together.
</Callout>
...
<Callout type="todo">
This needs to be completed with an equally simple example using the balls for example
</Callout>
</Callout>
...
<Callout type="todo">
This is in exercise sheet 2 and is a very common example of how certain probabilities are often misestimated intuitively.
