...
Let $X_1, X_2, \ldots$ be i.i.d. random variables with finite mean $\mu = \mathbb{E}[X_1] < \infty $. Then for every $\varepsilon > 0$ we have:

$$
\lim_{n \to \infty} \mathbb{P} \left( |\bar{S}_n - \mu| \geq \varepsilon \right) = 0
$$
...
$$
\mathbb{P}(|\bar{S}_n - p| \geq \varepsilon)
...
$$
\mathbb{P}(|\bar{S}_n - 0.7| \geq 0.05) \leq \frac{0.7 \times 0.3}{n \times 0.05^2}
$$
...
The idea here is that for almost every possible outcome $\omega$, the sequence $X_n(\omega)$ converges to $X(\omega)$. This is the "pointwise" convergence for random variables?

Lastly we have **convergence in distribution**. We say that a sequence of random variables $X_n$ converges in distribution to $X$ written as $X_n \xrightarrow{d} X$ if the cumulative distribution functions (CDFs) converge at all continuity points of the CDF of $X$. Formally, this means we have for all continuity points $x$ of $F_X$:
...
For normal variables, $Z_n \sim \mathcal{N}(0, 1)$ for all $n$!
...
$$
\mathbb{P}(Z \in [-2, 2]) \approx 0.954
$$

By the CLT,

$$
\lim_{n\to\infty} \mathbb{P}(n\mu - 2\sigma\sqrt{n} \leq S_n \leq n\mu + 2\sigma\sqrt{n}) \approx 95.4\%
$$
