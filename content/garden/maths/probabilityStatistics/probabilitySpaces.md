...
- A probability measure $\mathbb{P}$ which assigns a probability to each event in the event space.

This triplet is called a **probability space** and is denoted by $(\Omega, \mathcal{F}, \mathbb{P})$.
...
{{< figure
  src="/images/maths/probabilityEinsteinDice.jpeg"
  alt="Generated by AI"
...
- $\mathcal{F} = \{\emptyset, \{1,2,3,4,5,6\}\}$, the smallest sigma algebra
- $\mathcal{F} = \mathscr{P}(\Omega)$, i.e the power set of the sample space with $|\mathcal{F}|=2^6=64$ the largest sigma algebra
- $\mathcal{F} = \{\emptyset, \{1,2\}, \{3,4,5,6\}, \{1,2,3,4,5,6\}\}$
- $\mathcal{F} = \{\emptyset, \{1,2\}, \{3,4\}, \{5,6\}, \{1,2,3,4\}, \{1,2,5,6\}, \{3,4,5,6\}, \{1,2,3,4,5,6\}\}$
...
- $\mathcal{F} = \{\emptyset, \{1,2\}, \{3,4,5,6\}\}$, the certain event is not in the sigma algebra
- $\mathcal{F} = \{\emptyset, \{1,2\}, \{3,4\}, \{5,6\}\}$, the union of the events $\{3,4\}$ and $\{5,6\}$ is not in the sigma algebra
- $\mathcal{F} = \{\{1,2,4,5,6\}\}$, the complement of the event $\{1,2,4,5,6\}$ is not in the sigma algebra
{{< /callout >}}
...
$$
\Omega \in \mathcal{F} \Rightarrow \emptyset \in \mathcal{F} \text{ because } \Omega^c = \emptyset
$$
...
$$
A_1,A_2,... \in \mathcal{F} \Rightarrow \bigcap_{i=1}^{\infty} A_i \in \mathcal{F} \text{ because } \bigcap_{i=1}^{\infty} A_i = \left(\bigcup_{i=1}^{\infty} A_i^c\right)^c
$$
...
{{< figure
  src="/images/maths/probabilityVennEvents.png"
  alt="Some possible operations between two events."
...
{{< figure
  src="/images/maths/probabilityEventOperations.png"
>}}

The same goes for the relations between events.

{{< figure
  src="/images/maths/probabilityEventRelations.png"
>}}
...
$$
\mathbb{P}(A) = \sum_{i=1}^{\infty} \mathbb{P}(A_i) \text{ where } A = \bigcup_{i=1}^{\infty} A_i \text{ and } A_i \cap A_j = \emptyset \text{ for } i \neq j \text{ so a disjoint union}
$$
...
$$
\mathbb{P}(\{1\}) = \frac{1}{6}, \mathbb{P}(\{2\}) = \frac{1}{6}, \ldots, \mathbb{P}(\{6\}) = \frac{1}{6}
$$
...
If we know that the probability of an event $A$ is $p$ so $\mathbb{P}(A) = p$ then we can also quickly calculate the probability of the **complementary event** so if the event $A$ does not occur then the event $A^c$ must occur. The probability of the complementary event is then:

$$
\mathbb{P}(A^c) = 1 - \mathbb{P}(A) = 1 - p
$$
...
$$
\mathbb{P}(A) + \mathbb{P}(A^c) = \mathbb{P}(\Omega) = 1 \Rightarrow \mathbb{P}(A^c) = 1 - \mathbb{P}(A)
$$
...
This holds because probabilities are non-negative, so the intersection term $\mathbb{P}(A \cap B)$ is always non-negative. This means that the probability of the union of two events is at most the sum of their individual probabilities (when they are disjoint, this is actually an equality). This property is often referred to as the **subadditivity** of probability measures.

We can now extend this idea to the case of a finite or countably infinite collection of events which may not be disjoint. This then leads to **Boole's Inequality**. Which states that the probability of the union of a collection of events is at most the sum of their individual probabilities. So for any collection of events $A_1, A_2, \ldots \in \mathcal{F}$,
...
$$
\mathbb{P}\left(\bigcup_{i=1}^{n} A_i\right) = \mathbb{P}\left(\bigcup_{i=1}^{\infty} A_i\right)
$$

where for $i > n$, we set $A_i = \emptyset$. Since $\mathbb{P}(\emptyset) = 0$, the sum remains unchanged.

{{< callout type="proof" >}}
Let's prove the union bound by induction. Base case for $n = 1$:

$$
\mathbb{P}(A_1) \leq \mathbb{P}(A_1)
$$
...
$$
\mathbb{P}\left(\left(\bigcup_{i=1}^{n} A_i\right) \cup A_{n+1}\right) \leq \mathbb{P}\left(\bigcup_{i=1}^{n} A_i\right) + \mathbb{P}(A_{n+1})
$$
...
$$
\mathbb{P}(A) = \mathbb{P}\left( \bigcup_{k=1}^{n-\ell+1} A_k \right) \leq \sum_{k=1}^{n-\ell+1} \mathbb{P}(A_k)
$$
...
$$
\mathbb{P}(A_k) = \left(\frac{1}{6}\right)^{\ell}
$$

for every $k$. So,

$$
\mathbb{P}(A) \leq (n - \ell + 1) \left( \frac{1}{6} \right)^{\ell}
$$

If we now plug in $\ell = \lceil 7 \log n \rceil$, we get (using $n-\ell+1 \leq n$ for a rough upper bound):

$$
\mathbb{P}(A) \leq n \left( \frac{1}{6} \right)^{7 \log n}
$$

Recall that $a^{b \log n} = n^{b \log a}$, so:

$$
\left( \frac{1}{6} \right)^{7 \log n} = n^{7 \log(1/6)} = n^{-7 \log 6} \implies \mathbb{P}(A) \leq n \cdot n^{-7 \log 6}
$$
...
- **Increasing sequence:** If $A_1 \subseteq A_2 \subseteq \ldots$, then

$$
\lim_{n \to \infty} \mathbb{P}(A_n) = \mathbb{P}\left(\bigcup_{n=1}^\infty A_n\right)
$$

- **Decreasing sequence:** If $B_1 \supseteq B_2 \supseteq \ldots$, then
...
$$
\mathbb{P}\left(\bigcup_{n=1}^{\infty} A_n\right) = \sum_{n=1}^{\infty} \mathbb{P}(\tilde{A}_n) = \lim_{N \to \infty} \sum_{n=1}^{N} \mathbb{P}(\tilde{A}_n) = \lim_{N \to \infty} \mathbb{P}(A_N)
$$
...
$$
\mathbb{P}\left(\bigcap_{n=1}^{\infty} B_n\right) = 1 - \mathbb{P}\left(\bigcup_{n=1}^{\infty} B_n^c\right) = 1 - \lim_{n \to \infty} \mathbb{P}(B_n^c) = \lim_{n \to \infty} \mathbb{P}(B_n)
$$
{{< /callout >}}
...
$$
\begin{align*}
|\bigcup_{i=1}^{n} A_i| &= |A_1| + |A_2| + \ldots + |A_n| \\
&- \sum_{i < j} |A_i \cap A_j| + \sum_{i < j < k} |A_i \cap A_j \cap A_k| - \ldots + (-1)^{n+1} |A_1 \cap A_2 \cap \ldots \cap A_n| \\
&= \sum_{k=1}^{n} (-1)^{k+1} \sum_{1 \leq i_1 < i_2 < \ldots < i_k \leq n} |A_{i_1} \cap A_{i_2} \cap \ldots \cap A_{i_k}|
\end{align*}
$$
...
$$
\mathbb{P}(\{\omega_i\}) = p(\omega_i)= \frac{1}{20} \text{ with } i=1,2,...,20
$$
{{< /callout >}}
