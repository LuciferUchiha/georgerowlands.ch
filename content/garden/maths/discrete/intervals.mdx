...
We formally define the supremum and infimum of a set as the smallest upper bound and largest lower bound of a set, respectively. This is particularly useful when a set does not have a maximum or minimum but still has bounds. As the idea is that the supremum is the closest number to the maximum that is still an upper bound and the infimum is the closest number to the minimum that is still a lower bound. 

We formally define the supremum and infimum of a set $A \subseteq \mathbb{R}$ and $A \neq \emptyset$ as the real numbers $c \in \mathbb{R}$ such that:
...
<Callout type="proof">
Need to show that the supremum and infimum are well-defined, i.e. that they exist for any non-empty set $A \subseteq \mathbb{R}$ and that they are unique.
</Callout>
...
This can be intepreted as that the supremum is the smallest number such that if we subtract any positive number $\epsilon$ from it, then there is always an element in the set that is greater than this number. The similar applies to the infimum, where it is the largest number such that if we add any positive number $\epsilon$ to it, then there is always an element in the set that is smaller than this number.

<Callout type="todo">
What is the link min/max, supremum and infimum and the min/max of the set of all upper and lower bounds of a set?
</Callout>
...
- **Scalar multiplication**: $cA = \{ca \mid a \in A\}$
- **Set addition**: $A + B = \{a + b \mid a \in A, b \in B\}$

From this we can derive the following properties of the supremum and infimum:
- **Supremum of union**: $\sup(A \cup B) = \max(\sup A, \sup B)$
- **Infimum of intersection**: $\inf(A \cap B) = \min(\inf A, \inf B)$
- **Supremum and Infimum of scalar multiplication**: $\sup(cA) = c \cdot \sup A$ if $c > 0$ and $\sup(cA) = c \cdot \inf A$ if $c < 0$
- **Supremum and Infimum of set addition**: $\sup(A + B) = \sup A + \sup B$ and $\inf(A + B) = \inf A + \inf B$
