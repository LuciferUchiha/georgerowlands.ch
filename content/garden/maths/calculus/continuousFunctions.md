...
{{< figure
  src="/images/maths/continuityDelta.png"
  alt="The epsilon and delta neighborhoods of a point x_0 and the function f(x_0)."
...
{{< figure
  src="/images/maths/continuityIntermediateValueTheorem.png"
  alt="Visual representation of the intermediate value theorem."
...
{{< figure
  src="/images/maths/continuityIntermediateValueTheorem.png"
  alt="As it crosses the x-axis, it must have at least one root in the interval (a, b)."
...
Because $[a, b]$ is closed and bounded (hence compact), by the **Bolzano–Weierstrass theorem** there then exists a **convergent subsequence** $(x_{n_k})^{*k \geq 1}$ with $x_{n_k} \to x^* \in [a, b]$. Since $f$ is continuous at $x^*$, we have
...
{{< callout type="proof" >}}
$$
e^{\ln(x) + \ln(y)} = e^{\ln(x)}e^{\ln(y)} = xy \implies \ln(xy) = \ln(x) + \ln(y)
$$
{{< /callout >}}
