...
Below is a *self-contained, polished version* of the example—including a short comment on why every polynomial is continuous—ready to paste in.
...
<Callout type="proof">
This is all a bit confusing as I dont get the intuition. Let

$$
m=\min_{[a,b]} f,\qquad M=\max_{[a,b]} f .
$$

Because $f$ is continuous on the compact interval $[a,b]$, both exist and

$$
m\le f(x)\le M\quad\text{for all }x\in[a,b].
$$

Multiplying by the positive length $(b-a)$ and integrating gives
...
the *average value* of $f$ on $[a,b]$.  Then $m\le A\le M$.  continuity of $f$ implies the intermediate value property, so **there exists** $c\in[a,b]$ with $f(c)=A$, i.e.
...
- $f,g:[a,b]\to\mathbb{R}$ are continuous,
- $g(x)\ge0$ on $[a,b]$.

If $\int_a^b g(x)\,dx = 0$, then $g\equiv0$ (continuity), and the identity holds trivially for any $c$.
...
Multiply the pointwise inequality $m\le f(x)\le M$ by $g(x)$ and integrate:

$$
m\,G \;\le\; \int_a^b f(x)g(x)\,dx \;\le\; M\,G .
$$

Form the ratio

$$
A := \frac{\displaystyle\int_a^b f(x)g(x)\,dx}{\displaystyle\int_a^b g(x)\,dx},
\qquad
m \le A \le M .
$$

As before, the intermediate value theorem gives a point $c\in[a,b]$ with

$$
f(c)=A\;=\;\frac{\int_a^b f(x)g(x)\,dx}{\int_a^b g(x)\,dx}
$$
...
Main theorem of calculus states that the integral and the derivative are inverse operations. 
...
where $F$ is an antiderivative of $f$ on the interval $[a,b]$. 

Integrationsconstant $C$
...
We can now much easier calculate the integral of a function by finding an antiderivative of that function and then evaluating it at the endpoints of the interval. 
...
that is, when we have a composition of functions and the derivative of the inner function also appears in the integrand. A common example is
...
## Integration by Substitution of a Definite Integral
...
First we split the integrand as described above:
