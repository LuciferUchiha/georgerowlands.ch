...
<Callout type="proof">
For any partition $P$ with points $\delta$,
\[
...
If f is continuous then it is integratable. 

Before we prove it we define uniform continiouty. First a reminder of continuity using the epsilon-delta definition:


Then write out the definition of uniform continuity:

Notice it is almost the same as continiouty the order of the quantifiers is different. 

She doesnt actually show the proof. But if f is continuous then it is also uniformly continuous on a closed interval $[a,b]$.

<Callout type="example">
We want to look at the function f(x) = x^2 on the interval $[0,1]$.

as a reminder the definition of the integral is as the limit of the Riemann sums as the partition gets finer and finer. So we can take a equal length partition so $\delta(P_n)=\frac{1}{n}$ and then for our points we take x_k = k over n? All a bit werid setup.

```math
S(f,P_n,\delta^{(n)}) = \sum_{k=1}^{n} f\left(\frac{k}{n}\right) \cdot \frac{1}{n} = \sum_{k=1}^{n} \left(\frac{k}{n}\right)^2 \cdot \frac{1}{n} = \frac{1}{n^3} \sum_{k=1}^{n} k^2.
```
...
This means we get for the limit of the Riemann sums:

```math
\begin{align*}
\int_{0}^{1} x^2 \,dx &= \lim_{n\to\infty} S(f,P_n,\delta^{(n)}) \\
...
2. **Monotonicity**
   If $f(x)\le g(x)$ for every $x\in[a,b]$ then

   ```math
   \int_a^b\!f(x)\,dx\;\le\;\int_a^b\!g(x)\,dx.
   ```

5. **Absolute‑value inequality (triangle inequality)**

   ```math
   \Bigl|\int_a^b\!f(x)\,dx\Bigr|
   \;\le\;\int_a^b |f(x)|\,dx.
   ```

6. **Bounds via extrema**
   Let
