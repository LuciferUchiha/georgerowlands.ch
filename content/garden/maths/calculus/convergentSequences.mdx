...
<Callout type="todo">
Is this proof correct? I don't rly understand the archimedes principle part.
</Callout>
...
However, $n$ is positive and gets larger and larger, therefore the distance also gets larger and larger. So there exists no such $N_{\epsilon}$ that satisfies the inequality for all $n \geq N_{\epsilon}$. Therefore the sequence does not converge.
</Callout>
...
<Callout type="todo">
Does this come from probability theory? Where does this belong? For now I put it here because it is used in the proof of Euler's number.
</Callout>
...
babylonian method for finding square roots is a very old method that is still used today. It is based on the idea of finding the average of two numbers and then using this average to find a better approximation of the square root.

From notes more generally $a_n = 1 \over 2 (a_{n-1} + c \over a_{n-1})$ and $a_1 = c$ $c > 1$.

Used important technique of finding the limit based on the knowledge of it existing just like in the proof of the geometric sequence.
...
<Callout type="proof">
Todo
https://www.youtube.com/watch?v=R2AFZD0jiKQ
</Callout>
