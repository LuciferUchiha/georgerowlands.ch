...
When a infinite sequence is summed up it is called a [series](/garden/maths/calculus/sequencesAndSeries/series).
...
$$
\sum_{i=1}^{n} 2^i = 2 + 4 + 8 + ... + 2^n = 2^{n+1} - 2
$$

If it starts at 0 instead of 1 then the formula changes slightly to $2^{n+1} - 1$.

$$
\sum{i=0}^{n} ar^k = a + ar + ar^2 + ... + ar^n = \begin{cases}
a(n+1) & \text{if } r = 1 \\
